# Portfólio de Caio Henrique

Site estático em HTML, CSS e JavaScript puro, sem build e sem dependência externa além das
fontes do Google. No ar em [caiohenrique.dev](https://caiohenrique.dev).

| Arquivo | O que é |
|---|---|
| `index.html` | a página inteira, incluindo as ilustrações em SVG |
| `styles.css` | estilos, com a paleta e os tokens no bloco `:root` |
| `script.js` | rede de partículas do fundo, reveal ao rolar, tilt dos cards, rotator do hero |
| `assets/` | retrato, imagem de compartilhamento e screenshots dos projetos |
...
Atuação, 200×110 nas etapas), então para mexer basta editar os `x/y/width` direto no SVG. As
classes (`a-panel`, `a-fill`, `a-grad`, `a-wire`, `a-pulse`) são compartilhadas pelas sete
ilustrações.

## Screenshots dos projetos

Ficam em `assets/shots/`, capturadas dos sites reais em JPG 1600×1000. Para atualizar quando
algum deles mudar:
...
# depois converta para jpg, que é o formato apontado no HTML
```
...
Por ser estático, sobe em qualquer lugar: Vercel, Netlify, GitHub Pages, Hostinger via FTP ou
EasyPanel usando o `Dockerfile`, que serve a pasta com Nginx.
